...
function Events() {
  ...
  return (
    <section className="mt-20 flex h-auto w-full items-center justify-center sm:mt-[82px] md:mt-[122px]">
      <section className="z-50 flex w-full justify-center max-md:flex-col max-md:items-center max-sm:px-6 max-md:sm:px-10 md:justify-between md:gap-x-6 max-xl:md:px-6 xl:w-[1110px]">
        <motion.div
          id="imgsContainer"
        ...
        <motion.section
          className="flex flex-col justify-center max-md:w-full max-md:flex-col-reverse max-md:items-center max-md:text-center"
          initial={{ opacity: 0, x: 50 }}
          whileInView={{ opacity: 1, x: 0 }}
          viewport={{ once: true }}
          transition={{ type: "tween", delay: 0.3, duration: 0.4 }}
        >
          ...
          <ul
            id="eventsItems"
            className="relative mt-12 flex w-fit max-md:justify-between max-sm:flex-col max-sm:gap-y-4 sm:mt-[57px] sm:w-full md:mt-20 md:w-fit md:flex-col md:gap-3"
          >
            {eventsContainers.map((item, index) => (
              <div
                ...
                className={`flex cursor-pointer items-center text-lg uppercase text-[#4C4C4C] opacity-50 duration-300 hover:opacity-100 max-md:flex-col-reverse`}
              >
                <hr className="left-[-7rem] z-10 hidden h-[1px] w-[48px] border-beaver md:absolute md:w-[95px] xl:left-[-14.5rem]" />
                {item}
              </div>
            ))}
          </ul>
